...
const { divisions, insertManyDivision, deleteManyDivision } = require("../fixtures/divisions");
const { reviews, insertManyReview, deleteManyReview } = require("../fixtures/employeeReviews");
const { vacations, insertManyVacation, deleteManyVacation } = require("../fixtures/vacations");
...
describe("POST /division", () => {
  it("should return 201 if user is hr and add division successful", async () => {
    ...
    expect(res.body.data.employees).toEqual("12ab34cd56ef7890fe1d2c3b", "1a2b3c4d5e6f7a8b9c0d1e2f");
    expect(res.body.data.start_date).toBe("2021-01-01");
    expect(res.body.data.head_division).toBe("64991ad3185b61fae75951d1");
...
describe("DELETE /division/:id", () => {
  ...
  it("should return 401 if user is not logged in", async () => {
    const res = await supertest(app).delete(`/division/aaaaaaaaaaaaaaaaaaaaaaa`);
    expect(res.statusCode).toEqual(401);
  });
})
...
describe("PUT /review/:id", () => {
  ...
  it("should return 401 if user is not supervisor", async () => {
    ...
    expect(res.statusCode).toEqual(401);
    expect(res.body.message).toBe("Access Denied. Only supervisor is allowed to access");
  });

  it("should return 401 if user is not logged in", async () => {
    const res = await supertest(app).put(`/review/${review_id}`);
    expect(res.statusCode).toEqual(401);
  });
})
describe("GET /review", () => {
...
describe("GET /review/:id", () => {
  ...
  it("should return 401 if user is not logged in", async () => {
    const res = await supertest(app).get(`/review/${review_id}`);
    expect(res.statusCode).toEqual(401);
  });
...
describe("DELETE /review/:id", () => {
  ...
  it("should return 404 if user is supervisor but id review not found", async () => {
    const res = await supertest(app)
      .delete(`/review/aaaaaaaaaaaaaaaaaaaaaaaa`)
      .set("x-auth-token", supervisorToken);
    expect(res.statusCode).toEqual(404);
  ...
  it("should return 401 if user is not supervisor", async () => {
    const res = await supertest(app)
      .delete("/review/aaaaaaaaaaaaaaaaaaaaaaa")
      .set("x-auth-token", employeeToken)
    expect(res.statusCode).toEqual(401);
  });
  it("should return 401 if user is not logged in", async () => {
    const res = await supertest(app).delete(`/review/aaaaaaaaaaaaaaaaaaaaaaa`);
    expect(res.statusCode).toEqual(401);
  });
})
...
describe("GET /vacation/:id", () => {
  ...
  it("should return 401 if user is not logged in", async () => {
    const res = await supertest(app).get(`/review/${vacation_id}`);
    expect(res.statusCode).toEqual(401);
  });
...
describe("DELETE /vacation/:id", () => {
  ...
  it("should return 401 if user is not logged in", async () => {
    const res = await supertest(app).delete(`/vacation/aaaaaaaaaaaaaaaaaaaaaaa`);
    expect(res.statusCode).toEqual(401);
  });
})
